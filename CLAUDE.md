...
- 화면 간 공유 전역 상태는 [frontend/src/context/AppStateContext.tsx](frontend/src/context/AppStateContext.tsx)(`useAppState()` 훅)에 있다. 백엔드 연동은 **화면 단위로 진행 중**이며, 연동 완료된 화면(`LoginScreen`·`GuardianLoginScreen`·`ExerciseSelectScreen`)을 뺀 나머지는 아직 `AppStateContext`의 목업(`DEFAULT_PROFILE`/`DEFAULT_GUARDIAN`/`DEFAULT_SENIORS`/`DEFAULT_ALERTS` 등)을 쓴다. 백엔드 enum → 화면 표시 라벨 변환은 `Record<enum, label>` 타입으로 만들어 enum 확장 시 컴파일 타임에 누락이 드러나게 한다.
