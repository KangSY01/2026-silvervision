...
| AbilityHistoryScreen (내 운동 기록) | - (중간보고서 4.1절, 2026-09-21 자가평가 방식으로 전면 교체, 2026-09-22 달력 추가) | 완료 (월별 달력 + 선택일 세션·체감 난이도 표시) |
...
- **백엔드 API 연동 — 18개 화면 전체 완료**:
  ...
  **→ 18개 화면 전체 API 연동 완료**(`EntryScreen`은 조회 대상이 없는 진입 화면). `VoiceAssistantModal`만 설계 미확정으로 미마운트. `AppStateContext`에는 로그인·프로필 PATCH로 채워지는 `userProfile`/`guardianProfile`만 남고 목업 상수는 전부 제거됐다.
  - **`AbilityHistoryScreen` 전면 교체**(2026-09-21, physical_ability_log 목업 제거): 기존엔 `GET /senior/{id}/ability-log/`로 `rom_score`(관절 가동범위)·`completion_score`(동작 완성도)를 그래프로 그렸으나, 이 값들은 관절 각도 실측 소스가 없어(9장 — matcher가 boolean만 반환) 영원히 채워지지 않는 조회 전용 화면이었다. 이제는 실제로 채울 수 있는 지표로 교체했다 — 포커스마다 `GET /senior/{id}/sessions/` + `GET /exercises/`(exercise_id→이름 매핑, SeniorDetailScreen과 동일 패턴)를 조회해 완료된 세션(`completion_rate !== null`)만 로컬 상태로 모으고, 각 세션을 운동명·시각·**체감 난이도 자가평가**(`perceived_difficulty`, 아래 항목) 라벨로 보여준다. `ability-log` 엔드포인트·`PhysicalAbilityLog` 모델은 백엔드에 그대로 남아 있지만(비전 파이프라인이 붙으면 재사용 가능) 이 화면을 포함해 프론트 호출자가 없다.
    - **달력 UI 추가**(2026-09-22, 새 의존성 없이 순수 RN 컴포넌트로 구현): 완료 세션을 `created_at`(서버 UTC ISO)을 `new Date()`로 파싱한 뒤 로컬 getter로 뽑은 `'YYYY-MM-DD'` 키로 그룹화(`sessionsByDate`)하고, 월별 그리드(일요일 시작, `new Date(year, month+1, 0)`으로 월 마지막 날을 구해 연도 경계도 별도 분기 없이 처리)를 그려 날짜를 탭하면 그 날의 세션만 아래에 나열한다. 기록이 있는 날은 점 표시, 선택일은 테두리/배경 강조, 초기값은 오늘이 속한 달·오늘 날짜. 세션이 없는 날을 선택하면 "이 날은 운동 기록이 없어요" 안내로 대체 — 완료 세션이 아예 없는 신규 계정도 별도 분기 없이 이 안내로 자연스럽게 수렴하므로, 기존에 있던 전체-비어있음 전용 빈 상태 박스는 제거하고 달력+선택일 목록 하나로 통일했다. 기존의 "전체 세션 최신순 나열" 방식은 이 달력으로 완전히 대체됐다(날짜별로 나눠 보는 편이 더 찾기 쉽다는 판단).
  ...
  - **운동 세션 연동 흐름**(카메라 파이프라인 자체는 9장): `ExerciseProgressScreen` mount → `POST /senior/{id}/missions/`(scheduled_at=now, `senior`는 `ExerciseMissionCreateSerializer`가 필수라 본인 id를 body에 실음) → `POST /senior/{id}/sessions/`(`{mission}`, session_id를 ref에 보관). StrictMode/개발 모드 중복 호출은 `sessionStartRequestedRef`(effect 진입 즉시 세우는 동기 가드)가 막는다. 시퀀스 완주 시 자동, 또는 `__DEV__` 건너뛰기 버튼 → `ExerciseFeedback`로 `sessionId` + `result: ExerciseResult`(`ExercisePipeline`이 집계한 `completedSteps`/`totalSteps`/`elapsedMs`) 전달. `ExerciseFeedbackScreen` mount → `PATCH .../sessions/{id}/`(`completion_rate`·`accuracy_avg` 둘 다 `completedSteps/totalSteps` 비율 — 아래 참고) 후 응답(`ExerciseSessionCompleteResponse`의 `fruit_awarded`/`fruit_count`/`today_completed`/`daily_goal`)으로 결과·열매 카드를 그린다(프론트가 임의로 "+1"을 띄우지 않고 `fruit_awarded`를 따른다). X 버튼 이탈은 `goBack()`만 하고 세션을 미완료(completion_rate=null)로 남긴다(백엔드가 완료로 집계하지 않음).
    ...
    - **체감 난이도 자가평가**(2026-09-21 신설, `perceived_difficulty`): 완료 PATCH 직후 화면 하단에 "오늘 운동, 어떠셨어요?" 5단계(매우 쉬웠다~매우 힘들었다) 버튼을 보여준다. 선택 시 완료 PATCH와 **분리된** 후속 `PATCH .../sessions/{id}/ { perceived_difficulty: N }`을 보낸다(같은 `ExerciseSessionCompleteSerializer`가 세 필드 모두 optional이라 그대로 재사용). 평가하지 않고 화면을 벗어나도(뒤로가기 등) 문제없음을 "선택 안 하셔도 괜찮아요" 문구로 명시. 실패 시 `errorBox` 패턴(danger 색상)으로 안내하고 버튼을 다시 누르면 재시도된다. `AbilityHistoryScreen`이 이 값을 표시한다.
...
- **아직 실제 값이 아닌 것**: `PoseFeedback.deviation`(관절별 편차)은 placeholder다. `matcher.ts`의 `matchesPose()`가 boolean만 반환해 각도 차이를 노출하지 않기 때문이며, 같은 이유로 `accuracy_avg`도 현재 `completion_rate`와 같은 값이다. matcher가 각도 차이를 함께 반환하도록 확장돼야 갈라진다. `rom_score`/`completion_score`(`physical_ability_log`)도 같은 이유로 채울 수 없어, 2026-09-21에 체감 난이도 자가평가(`perceived_difficulty`)로 대체했다(7장 `AbilityHistoryScreen` 전면 교체 참고) — matcher 확장이 아니라 사용자 자가평가로 우회한 것이라, matcher가 나중에 실측값을 반환하게 되어도 이 자가평가 자체는 그대로 유지되는 별개 지표다.
