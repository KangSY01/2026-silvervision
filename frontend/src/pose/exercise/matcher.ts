 ...
 * 그 항목들이 전부 angleToleranceDeg 이내여야 매칭으로 본다.
 *
 * refAngles는 포즈별로 활성화된 관절만 값을 갖고 나머지는 null로 마스킹되어 들어오므로
 * (reference-poses.ts의 buildMatchTargets), minVisibleJoints를 그 포즈의 활성 관절
...
export function matchesPose(
  ...
  for (let i = 0; i < refAngles.length; i++) {
    ...
    comparable++;
    if (Math.abs(live - ref) > angleToleranceDeg) return false;
  }
