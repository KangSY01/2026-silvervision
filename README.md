...
[발표 자료](docs/03.발표자료/발표자료.pdf)
[포스터](docs/02.포스터/포스터파일.pdf)

### 6.2 시연 영상

[2026 전기 졸업과제 01 실버비전](https://youtu.be/90oTwhME3dg)
...
박소영(프론트엔드): 고령화 사회에서 시니어의 안전과 건강을 기술로 지키고자 '실버비전' 프로젝트를 시작했습니다. 낙상 감지 및 맞춤형 홈트레이닝 시스템을 구축하며, 기술적 구현을 넘어 사용자 관점의 데이터 설계와 시스템 아키텍처 통합의 중요성을 깨달았습니다. 단순한 기술 적용에 그치지 않고, 복잡한 지표와 구조를 직관적인 서비스로 시각화하며 실질적인 헬스케어 가치를 만들어내는 역량을 키운 뜻깊은 경험이었습니다.

주은택(AI/비전): 비전 파트를 맡아 자세 추정 모델을 다루고, 운동 자세 매칭과 낙상 감지 판정 로직을 앱에 이식하면서 학습 환경과 실기기 환경이 많이 다르다는 점을 배웠습니다. 판정 결과를 알림으로 넘기며 프론트 및 백엔드와 인터페이스를 맞춘 경험도 남았습니다. 특히 낙상은 연속 감지와 파라미터를 다듬는 과정에서 많은 시간을 쓰며 실전 감각을 익힐 수 있는 좋은 시간이었습니다.
